...
# Forked action
  ...
  # Try to pull pre-built image from GitHub Container Registry (METRICS_USE_PREBUILT_IMAGE で無効化可能)
  if [[ ! $METRICS_USE_PREBUILT_IMAGE =~ ^([Ff]alse|[Oo]ff|[Nn]o|0)$ ]]; then
    ...
    echo "Pre-built image disabled via METRICS_USE_PREBUILT_IMAGE, will build locally"
  fi
fi
